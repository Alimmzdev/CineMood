...
fun <T> executeToFlow(
    ...
    block: suspend () -> T
): Flow<dev.alimmz.cinemood.core.domain.common.BaseResult<T>> = flow {
    println("Starting: $operationName on $dispatcher")
    try {
        val result = block()
        println("Finished: $operationName - Success")
        emit(dev.alimmz.cinemood.core.domain.common.BaseResult.Success(result))
    } catch (e: Exception) {
        println("Finished: $operationName - Error: ${e.message}")
        emit(dev.alimmz.cinemood.core.domain.common.BaseResult.Error(e))
    }
}.flowOn(dispatcher)
    .catch { e ->
        emit(dev.alimmz.cinemood.core.domain.common.BaseResult.Error(e as? Exception ?: Exception(e)))
    }
...
fun <T> executeToIoFlow(
    operationName: String,
    block: suspend () -> T
): Flow<dev.alimmz.cinemood.core.domain.common.BaseResult<T>> =
    executeToFlow(
        operationName,
        Dispatchers.Default,
        block
...
suspend fun <T : BaseRepository> T.executeWithLogging(
    operationName: String,
    block: suspend T.() -> dev.alimmz.cinemood.core.domain.common.BaseResult<*>
): dev.alimmz.cinemood.core.domain.common.BaseResult<*> {
